...
from streamlit_lottie import st_lottie # to display lottie animation
import json
import pandas as pd
...
geolocator = Nominatim(user_agent="App", timeout=10)  # Adjust timeout as needed
...
# sql connector
connection = pymysql.connect(host='localhost',user='root',password='2308',db='cv')
...
def run():
    ...
    with open("./Logo/ResAnimation.json", "r") as f:
        animation_data = json.loads(f.read())

    with st.sidebar:
        # Display the animation in the sidebar
        st_lottie(animation_data, height=200, width=250, key="animation")  # Adjust height and width as needed
    ...
    link = '<b>Built with 🤍 by <a href="https://www.linkedin.com/in/kunal23/" style="text-decoration: none; color: #4085f5;">Kunal Chaudhari</a></b>' 
    ...
    # Reset any app state variables or perform actions before refresh (optional)

    # Trigger app refresh using st.experimental_rerun()
    ...
    ###### CODE FOR FEEDBACK SIDE ######
        ...
        fig = px.pie(values=plotfeed_data.feed_score.value_counts(), names=plotfeed_data.feed_score.unique(), title="Chart of User Rating Score From 1 - 5", color_discrete_sequence=px.colors.sequential.Aggrnyl)
        st.plotly_chart(fig)
    ...
    ###### CODE FOR ABOUT PAGE ######
        ...
        <p align="justify">
            Built with 🤍 by 
            <a href="https://www.linkedin.com/in/kunal23/" style="text-decoration: none; color: red;">KC</a>
        </p>
    ...
    ###### CODE FOR ADMIN SIDE (ADMIN) ######
        ...
        if st.button('Login'):
            ...
            ## Credentials 
            if ad_user == 'root' and ad_password == '2308':
